...
class DSPyReactMCPSystem:
    ...
    async def _initialize_system(self):
        """Initialize the system without starting the main loop."""
    ...
    async def start(self):
        """Start the DSPy React MCP system with main loop."""
    ...
    def agents(self) -> List[Agent]:
        ...
        def collect_agents(agent: Agent) -> List[Agent]:
            agents = [agent]
            for sub_agent in agent.agents:
                agents.extend(collect_agents(sub_agent))
            return agents
        
        return collect_agents(self.boss_agent)
    
    @property
    def human_system(self):
        """Get the human system."""
        return self.human_agent_system
    ...
    async def get_system_status(self) -> Dict[str, Any]:
        ...
        for agent in self.agents:
            total_tasks += len(agent.current_tasks) + len(agent.completed_tasks) + len(agent.task_queue)
